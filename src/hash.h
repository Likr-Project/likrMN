...
template <typename T1, typename T2>
inline uint256 HashS(const T1 p1begin, const T1 p1end, const T2 p2begin, const T2 p2end)
{
    static const unsigned char pblank[1] = {};
    uint256 result;
    CSHA256().Write(p1begin == p1end ? pblank : (const unsigned char*)&p1begin[0], (p1end - p1begin) * sizeof(p1begin[0])).Write(p2begin == p2end ? pblank : (const unsigned char*)&p2begin[0], (p2end - p2begin) * sizeof(p2begin[0])).Finalize((unsigned char*)&result);
    return result;
}
...
public:
    ...
    CHashSWriter(int nTypeIn, int nVersionIn) : nType(nTypeIn), nVersion(nVersionIn) {}

    CHashSWriter& write(const char* pch, size_t size)
    {
        ctx.Write((const unsigned char*)pch, size);
        return (*this);
    }
    ...
    template <typename T>
    CHashSWriter& operator<<(const T& obj)
    {
        // Serialize to this stream
        ::Serialize(*this, obj, nType, nVersion);
        return (*this);
...
template <typename T>
uint256 SerializeHashS(const T& obj, int nType = SER_GETHASH, int nVersion = PROTOCOL_VERSION)
{
    CHashSWriter ss(nType, nVersion);
    ss << obj;
    return ss.GetHash();
}
...
/* ----------- Keccak256 Hash ------------------------------------------------ */
template <typename T1>
inline uint256 HashKeccak256(const T1 pbegin, const T1 pend)
{
    static unsigned char pblank[1];
    ...
    sph_keccak256_close(&ctx_keccak, hash.begin());
