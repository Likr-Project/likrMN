...
{
    if (blockHeight >= 0 && blockHeight < 3000) {
      switch(vin_val) {
          case 1000000 * COIN: return 1; 
          case 3000000 * COIN: return 2;
          case 5000000 * COIN: return 3;          
      }
    } else {
      switch(vin_val) {
          case 1000000 * COIN: return 1; 
...
*/
    ...
    // should be at least not earlier than block when 1000 LKRM tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && mi->second) {
        CBlockIndex* pMNIndex = mi->second;                                                        // block for 1000 LKRM tx -> 1 confirmation
