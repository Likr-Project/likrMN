...
#if defined(NDEBUG)
#error "LKRM cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ulkrm) are considered zero fee (for relaying and mining)
...
// lkrm: total coin age spent in transaction, in the unit of coin-days.
...
CAmount GetBlockValue(int nHeight)
{
  // anti instamine
	...
	} else if (nHeight < 43800*1 ) {
        nSubsidy = 31.7 * COIN;

	} else if (nHeight < 43800*2 ) {
        nSubsidy = 66.6 * COIN;

	} else if (nHeight < 43800*3 ) {
        nSubsidy = 79.3 * COIN;

	} else if (nHeight < 43800*4 ) {
        nSubsidy = 84.3 * COIN;

	} else if (nHeight < 43800*5 ) {
        nSubsidy = 88.5 * COIN;
        
	} else if (nHeight < 43800*6 ) {
        nSubsidy = 97.7 * COIN;

	} else if (nHeight < 43800*7 ) {
        nSubsidy = 103.1 * COIN;

	} else if (nHeight < 43800*8 ) {
        nSubsidy = 109.4 * COIN;

	} else if (nHeight < 43800*9 ) {
        nSubsidy = 118.6 * COIN;

	} else if (nHeight < 43800*10 ) {
        nSubsidy = 126.5 * COIN;

	} else if (nHeight < 43800*11 ) {
        nSubsidy = 139.8 * COIN;

	} else if (nHeight < 43800*12 ) {
        nSubsidy = 145.9 * COIN;

	} else if (nHeight < 43800*13 ) {
        nSubsidy = 141.5 * COIN;

	} else if (nHeight < 43800*14 ) {
        nSubsidy = 137.2 * COIN;
        
	} else if (nHeight < 43800*15 ) {
        nSubsidy = 133.1 * COIN;

	} else if (nHeight < 43800*16 ) {
        nSubsidy = 129.1 * COIN;

	} else if (nHeight < 43800*17 ) {
        nSubsidy = 125.3 * COIN;

	} else if (nHeight < 43800*18 ) {
        nSubsidy = 121.5 * COIN;

	} else if (nHeight < 43800*19 ) {
        nSubsidy = 117.9 * COIN;

	} else if (nHeight < 43800*20 ) {
        nSubsidy = 114.3 * COIN;

	} else if (nHeight < 43800*21 ) {
        nSubsidy = 110.9 * COIN;

	} else if (nHeight < 43800*22 ) {
        nSubsidy = 107.6 * COIN;

	} else if (nHeight < 43800*23 ) {
        nSubsidy = 104.3 * COIN;

	} else if (nHeight < 43800*24 ) {
        nSubsidy = 101.2 * COIN;                                                                                                                                                                                        
...
bool IsInitialBlockDownload()
{
    ...
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
                  pindexBestHeader->GetBlockTime() < GetTime() - (120 * 24 * 60 * 60)); 
    if (!state)
        lockIBDState = true;
    return state;
}
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    // lkrm: track money supply and mint amount info
...
CBlockIndex* AddToBlockIndex(const CBlock& block)
{
    ...
    if (miPrev != mapBlockIndex.end()) {
        ...
        // lkrm: compute chain trust score
        pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

        // lkrm: compute stake entropy bit for stake modifier
        if (!pindexNew->SetStakeEntropyBit(pindexNew->GetStakeEntropyBit()))
            LogPrintf("AddToBlockIndex() : SetStakeEntropyBit() failed \n");

        // lkrm: record proof-of-stake hash value
        if (pindexNew->IsProofOfStake()) {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake() && chainActive.Height() > DEFAULT_BLOCK_SPAM_FILTER_START && IsSporkActive(SPORK_9_ACCEPT_BLOCK_SPAM_FILTER_ENFORCEMENT) ) {
            LOCK(cs_main);
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
...

    {
        ...
        if (pfrom && GetBoolArg("-blockspamfilter", DEFAULT_BLOCK_SPAM_FILTER)) {
          ...
          if (chainActive.Height() > DEFAULT_BLOCK_SPAM_FILTER_START && IsSporkActive(SPORK_8_PROCESS_BLOCK_SPAM_FILTER_ENFORCEMENT)) {
