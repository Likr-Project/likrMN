...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::LKRM)
    {
    }
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->label_LKRM4->setVisible(showImmature || showWatchOnlyImmature);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    if (model && model->getOptionsModel()) {
        ...
        connect(ui->blabel_LKRM, SIGNAL(clicked()), this, SLOT(openMyAddresses()));

    }

    // update the display unit, to not use the default ("LKRM")
    updateDisplayUnit();
}
...
void OverviewPage::updateMasternodeInfo()
{
  ...
  if (chainActive.Height() >= 0 && chainActive.Height() < 3000) {
    ui->label_lcolat->setText("1,000,000 LKRM");
  } else {
    ui->label_lcolat->setText("1,000,000 LKRM");
  }
...
double BlockRewardLKRM =  static_cast<double>(BlockReward/COIN);
//int64_t LKRMSupply = chainActive.Tip()->nMoneySupply / COIN;
...
ui->label_CurrentBlockReward_value->setText(QString::number(BlockRewardLKRM));
//ui->label_LKRMSupply_value->setText(QString::number(LKRMSupply));
